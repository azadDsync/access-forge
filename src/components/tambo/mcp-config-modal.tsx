...
export const McpConfigModal = ({
  ...
  const [mcpServers, setMcpServers] = React.useState<McpServerInfo[]>(() => {
    ...
    try {
      const parsed = JSON.parse(localStorage.getItem("mcp-servers") ?? "[]");
  ...
  React.useEffect(() => {
    const updateServers = () => {
      ...
      try {
        const parsed = JSON.parse(savedServersData);
        const servers = normalizeMcpServers(parsed);
        setServers(dedupeMcpServers(servers));
      } catch (e) {
        console.error("Failed to parse saved MCP servers", e);
        setServers([]);
      }
...
function normalizeMcpServers(raw: unknown): McpServerInfo[] {
  ...
  return raw
    .map((server) => {
      ...
      if (server && typeof server === "object" && "url" in server) {
        const { url, name, description, transport, serverKey } =
          server as Partial<McpServerInfo> & {
            url: string;
            headers?: Record<string, string>;
          };

        const customHeaders =
          "customHeaders" in server
            ? (server as McpServerInfo).customHeaders
...
function dedupeMcpServers(servers: McpServerInfo[]): McpServerInfo[] {
  const uniqueUrls = new Set<string>();
  return servers.filter((server) => {
    if (uniqueUrls.has(server.url)) return false;
    uniqueUrls.add(server.url);
    return true;
  });
}
